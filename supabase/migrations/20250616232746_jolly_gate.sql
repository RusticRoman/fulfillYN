/*
  # Create 3PL Form Tables

  1. New Tables
    - `capabilities` - 3PL service capabilities and offerings
    - `compliance` - compliance and certification information  
    - `tech_stack` - technology and integration information
  ...
  2. Security
    ...
    - Add policies for users to manage their own company data
    - Add policies for admins to view all data

  3. Relationships
    - All tables link to companies via company_id foreign key
...
CREATE TABLE IF NOT EXISTS compliance (
  ...
  -- Ensure one compliance record per company
  UNIQUE(company_id)
);
...
CREATE TABLE IF NOT EXISTS tech_stack (
  ...
  -- Ensure one tech stack record per company
  UNIQUE(company_id)
);
...
CREATE TABLE IF NOT EXISTS performance_metrics (
  ...
  -- Ensure one performance metrics record per company
  UNIQUE(company_id)
);
...
CREATE TABLE IF NOT EXISTS media_assets (
  ...
  -- Ensure one media assets record per company
  UNIQUE(company_id)
);
